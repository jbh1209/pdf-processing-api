...
class GhostscriptService:
    ...
    async def flatten_transparency(
        ...
        logger.info("Running Ghostscript transparency flatten")
    ...
    async def rasterize_pages(
        ...
        if not pages:
            ...
            if proc.returncode != 0:
                error_msg = stderr.decode() if stderr else "Unknown error"
                raise PDFProcessingError(f"Ghostscript page count failed: {error_msg}")
        ...
        for page_num in pages:
            ...
            if process.returncode != 0:
                ...
                raise PDFProcessingError(
                    f"Ghostscript rasterize failed on page {page_num}: {error_msg}"
                )
